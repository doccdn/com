...
(function () {
	var apiFactory = function($timeout, $q, $http, fnService, dbFactory){
		...
		api.post = function(url, params, opt){
			return api.http({url: url, method: "POST", data: JSON.stringify(params)}, opt); //application/json
		} 
		
		api.put = function(url, params, opt){
			return api.http({url: url, method: "PUT", data: JSON.stringify(params)}, opt); //application/json
		}
		
		api.del = function(url, params, opt){
			return api.http({url: url, method: "DELETE", params: params}, opt); //application/x-www-form-urlencoded
		}
		...
		api.jsonp = function(url, params, opt){
			...
			return api.http({url: url, method: "JSONP", params: params}, opt); // return $_GET[callback]."(".json_encode($data).")"
		}
		
		api.http = function(config, currOpt){
